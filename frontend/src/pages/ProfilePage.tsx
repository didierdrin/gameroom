  ...
  const fetchUserProfile = async (targetUsername?: string, showLoading = true) => {
    ...
    try {
      ...
      if (!isOwn && targetUsername) {
        const lookupResponse = await fetch(`https://alu-globe-gameroom.onrender.com/user/username/${targetUsername}`, {
          method: 'GET',
          headers: {
                        ...
                        <div className="flex-1">
                          <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                            <div 
                              ...
                              style={{ width: `${Math.round(stat.winRate)}%` }}
                            ></div>
                          </div>
                        </div>
              ...
              {userData.favoriteGames.map((game) => (
                <div key={game.gameType} className="bg-gray-700/30 rounded-lg p-4">
                  ...
                  <div className="text-sm text-gray-400">
                    ...
                    <div>Win Rate: {Math.round(game.winRate)}%</div>
