...
export const MyGameRoomsPage = ({ onJoinRoom }: any) => {
  ...
  const [hostedRooms, setHostedRooms] = useState<GameRoom[]>(MOCK_MY_HOSTED_ROOMS);
  const [joinedRooms, setJoinedRooms] = useState<GameRoom[]>(MOCK_MY_JOINED_ROOMS);
  ...
  useEffect(() => {
    ...
    socket.on('myGameRoomsList', (data: { hosted: GameRoom[]; joined: GameRoom[] }) => {
      setHostedRooms(data.hosted || MOCK_MY_HOSTED_ROOMS);
      setJoinedRooms(data.joined || MOCK_MY_JOINED_ROOMS);
    });
