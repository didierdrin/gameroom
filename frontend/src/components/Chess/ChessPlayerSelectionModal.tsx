...
export const ChessPlayerSelectionModal: React.FC<ChessPlayerSelectionModalProps> = ({
  ...
  const availablePlayers = players.filter(player => player.id); // Basic validation

  const handleConfirm = () => {
    if (selectedPlayer1 && selectedPlayer2 && selectedPlayer1 !== selectedPlayer2) {
      console.log('Confirming chess player selection:', {
        player1Id: selectedPlayer1,
        player2Id: selectedPlayer2,
        player1Name: getPlayerName(selectedPlayer1),
        player2Name: getPlayerName(selectedPlayer2)
      });
  ...
  const handlePlayerSelect = (playerId: string, isPlayer1: boolean) => {
    console.log('Player selection:', { playerId, isPlayer1, availablePlayers: players.length });
    
    if (isPlayer1) {
      setSelectedPlayer1(playerId);
      // If player2 was the same as player1, clear it
      if (selectedPlayer2 === playerId) {
        setSelectedPlayer2('');
      }
    } else {
      setSelectedPlayer2(playerId);
      // If player1 was the same as player2, clear it
  ...
  const handleClose = () => {
    // Reset selections when closing without confirming
    setSelectedPlayer1('');
    setSelectedPlayer2('');
    onClose();
  };
