...
export class GameService {
  ...
  private calculatePlayerPoints(gameState: GameState): PlayerPoints[] {
    ...
    // --- Uno Scoring ---
    else if (gameState.gameType === 'uno') {
      const unoPointsMap: { [key: number]: number } = {
        1: 20, 2: 15, 3: 10, 4: 5
      };
      ...
      const sortedPlayers = [...gameState.players].sort((a, b) => {
        ...
        const aCards = (a as any).cards?.length || 0;
        const bCards = (b as any).cards?.length || 0;
      ...
      return sortedPlayers.map((player, index) => ({
        playerId: player.id,
        name: player.name,
        color: (player as any).color || 'blue', // Default color if not present
        position: index + 1,
        points: unoPointsMap[index + 1] || 0
      }));
    }
    ...
    // --- Chess Scoring ---
      ...
      return gameState.players.map(player => {
        ...
        return {
          playerId: player.id,
          name: player.name,
          color: (player as any).chessColor || 'gray',
