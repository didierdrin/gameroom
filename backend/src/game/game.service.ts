...
export class GameService {
  ...
  //       (player as any).isSpectator = false as any;
  //     } else if (player.id === data.player2Id) {
  //       player.chessColor = 'black';
  //       (player as any).isSpectator = false as any;
  //     } else {
  //       (player as any).isSpectator = true as any;
  //       delete (player as any).chessColor;
  //     }
  //   });
...
async makeChessMove(data: { roomId: string; playerId: string; move: string }) {
  ...
  // Validate it's the player's turn
  if (gameState.currentTurn !== data.playerId) {
    throw new Error(`Not your turn. Current turn: ${gameState.currentTurn}`);
  }
  ...
  try {
    ...
    if (chess.isGameOver()) {
      ...
      // Switch turns - find the next player
      const currentPlayerIndex = gameState.players.findIndex(p => p.id === data.playerId);
      const nextPlayerIndex = (currentPlayerIndex + 1) % gameState.players.length;
      
      // Find the next player who has a chess color (in case of spectators)
      let nextPlayer = gameState.players[nextPlayerIndex];
      for (let i = 0; i < gameState.players.length; i++) {
        const candidatePlayer = gameState.players[(nextPlayerIndex + i) % gameState.players.length];
        if (candidatePlayer.chessColor && candidatePlayer.id !== data.playerId) {
...
//       if (scheduledTimeCombined <= new Date()) throw new Error('Scheduled time must be in the future');
//     }
//     const maxPlayers = createGameDto.gameType.toLowerCase() === 'chess' ? 10 : 20;
//     const gameRoom = new this.gameRoomModel({
//       roomId,
//       name: createGameDto.name,
//       gameType: createGameDto.gameType.toLowerCase(),
...
//   private async initializeGameState(roomId: string, hostId: string, roomName: string, gameType: string, triviaTopic: string) {
//     const hostName = await this.getPlayerName(hostId);
//     const colors = ['red', 'blue', 'green', 'yellow'];
//     let initialGameState: GameState;
...
//             id: hostId, 
//             name: hostName, 
//             isSpectator: false // Initially everyone is a spectator until players are selected
//           }],
//           currentTurn: '', // No turn until players are selected
//           currentPlayer: 0, // No current player until game starts
//           gameStarted: false,
//           gameOver: false,
//           winner: null,
//           roomName,
...
//           if (!gameState.players.find(p => p.id === joinGameDto.playerId)) {
//             const playerName = joinGameDto.playerName || await this.getPlayerName(joinGameDto.playerId);
//             gameState.players.push({ id: joinGameDto.playerId, name: playerName, color: nextColor, coins: [0, 0, 0, 0] });
//           }
//           if (!gameState.coins) gameState.coins = {};
//           if (!gameState.coins[joinGameDto.playerId]) {
//             gameState.coins[joinGameDto.playerId] = [0, 0, 0, 0];
...
//           if (!gameState.players.find(p => p.id === joinGameDto.playerId)) {
//             const playerName = joinGameDto.playerName || await this.getPlayerName(joinGameDto.playerId);
//             gameState.players.push({ id: joinGameDto.playerId, name: playerName, score: 0 });
//           }
//           if (gameRoom.gameType === 'trivia' && gameState.triviaState) {
//             gameState.triviaState.scores[joinGameDto.playerId] = 0;
//             gameState.triviaState.answers[joinGameDto.playerId] = null;
...
//       const gameState = await this.getGameState(roomId); // Get latest state
...
//     // Basic validation - game must be started
//     if (!gameState.gameStarted) {
//       throw new Error('Game has not started yet');
//     }

//     if (!gameState.chessPlayers) {
//       throw new Error('Chess players not selected');
//     }
...
//     // Validate that the player is one of the selected chess players
//     const isPlayer1 = gameState.chessPlayers.player1Id === data.playerId;
//     const isPlayer2 = gameState.chessPlayers.player2Id === data.playerId;
...
//       chessGame.load(gameState.chessState.board);
//     }

//     // Parse the move (format: "e2e4" or "e2e4q" for promotion)
...
//     // Update board state
//     gameState.chessState.board = chessGame.fen();
//     gameState.chessState.moves = gameState.chessState.moves || [];
//     gameState.chessState.history = gameState.chessState.history || [];
    
//     // Add move to history
//     gameState.chessState.moves.push(move.san);
//     gameState.chessState.history.push({
//       move: move.san,
//       fen: chessGame.fen(),
...
//     console.log('Chess move saved:', {
//       move: move.san,
//       newFen: chessGame.fen(),
//       moveCount: gameState.chessState.moves.length
//     });
...
//         // The player who just moved wins (since the other player is in checkmate)
//         gameState.winner = data.playerId;
//         gameState.winCondition = 'checkmate';
...
//           gameState.winCondition = 'threefold_repetition';
//         } else {
//           gameState.winCondition = 'draw';
//         }
//       }
      
//       console.log('Chess game ended:', {
//         winner: gameState.winner,
//         condition: gameState.winCondition
//       });
...
//     // Emit updated board state to all players in the room
//     if (this.server) {
//       this.server.to(data.roomId).emit('chessBoardUpdate', {
//         board: chessGame.fen(),
//         move: move.san,
//         gameState: {
...
//           chessPlayers: gameState.chessPlayers,
//           currentTurn: gameState.currentTurn, // Include updated turn in emission
...
//       if (redisState) {
//         const parsedState: GameState = JSON.parse(redisState);
//         // Ensure all player names are usernames (refetch if needed)
//         const playersWithNames = await Promise.all(parsedState.players.map(async (player, index) => ({
//           ...player,
//           name: player.id.startsWith('ai-') ? `AI ${index + 1}` : await this.getPlayerName(player.id),
//         })));
//         console.log(`Retrieved game state from Redis for room ${roomId}:`, {
//           currentTurn: parsedState.currentTurn,
//           gameStarted: parsedState.gameStarted,
//           gameOver: parsedState.gameOver,
//           gameType: parsedState.gameType,
//           players: playersWithNames.map((p: any) => ({ 
//             id: p.id, 
//             chessColor: p.chessColor,
//             color: p.color,
//             score: p.score
...
//       switch (room.gameType) {
//         case 'chess':
//           const chessPlayers = await Promise.all(room.playerIds.map(async (playerId) => ({
//             id: playerId,
...
//             currentPlayer: 0,
//             currentTurn: room.playerIds?.[0] || '',
//             players: chessPlayers.map((player, index) => ({
//               ...player,
//               chessColor: undefined, // Colors set later on selection
...
//         case 'trivia':
//           const questions = room.gameType === 'kahoot' ? await this.fetchTriviaQuestions() : [];
//           const kahootTriviaPlayers = await Promise.all(room.playerIds.map(async (playerId) => ({
//             id: playerId,
//             name: await this.getPlayerName(playerId),
//             score: 0,
//           })));
...
//           break;
//         default:
//           const ludoPlayers = await Promise.all(room.playerIds.map(async (playerId, index) => ({
//             id: playerId,
//             name: await this.getPlayerName(playerId),
